"""
***大模型私有化部署服务---vllm版本需要在Linux内核运行的四模式代码，采用类方法封装***
...
from vllm import LLM, SamplingParams# vLLM 加速库，提供高性能的语言模型推理（可选，需安装 vllm 包,须在Linux系统上运行）
...
class Config:
    ...
    max_context_length: int = 5000# 模型最大上下文长度（包含输入和输出）
    ...
    ui_path: str = "/ui"  # Gradio 挂载路径（ui+api 模式）
...
# ================================================================
class ChatBot:
    """基于 DeepSeek-R1-Distill-Qwen-1.5B 和 vLLM 的多轮对话机器人"""
    def __init__(self, config: Config, logger: logging.Logger):
        self.config = config
        self.logger = logger
        self.model_path = str(config.model_path)
        self._lock = threading.Lock()
        self.logger.info("正在从本地加载 tokenizer...")
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.model_path, trust_remote_code=True
        )
        self.logger.info("正在初始化 vLLM 模型...")
        # vLLM LLM实例创建
        # 支持的参数可以根据实际需求调整
        self.llm = LLM(self.model_path)
        self.logger.info("vLLM 模型加载完毕 ✅")

    def _trim_context(self, history: list[dict]) -> list[dict]:
        """
        裁剪历史对话保证上下文长度不超过最大。
        vLLM 这边我们统一使用message拼接成字符串，所以裁剪策略可保持简单
        """
        trimmed = history.copy()
        while True:
            # 拼接文本计算token长度
            text = self._build_prompt(trimmed)
            tokens_len = len(self.tokenizer.encode(text))
            if tokens_len + self.config.max_new_tokens <= self.config.max_context_length:
                break
            if len(trimmed) <= 1:
                break
            trimmed.pop(0)
            # 如果裁剪后首条是 assistant，丢弃保证user开头
            if trimmed and trimmed[0]["role"] == "assistant":
                trimmed.pop(0)
            self.logger.warning(f"上下文过长，已丢弃最早的对话（现token数：{tokens_len}）")
        return trimmed

    def _build_prompt(self, history: list[dict]) -> str:
        """
        根据多轮对话历史构造vLLM的完整prompt文本。
        此处示例采用简单拼接用户和助手角色文本，也可以基于模型需要自定义模板。
        """
        prompt = ""
        for turn in history:
            role = turn["role"]
    ...
    def chat(
        ...
        trimmed_history = self._trim_context(history)

        prompt = self._build_prompt(trimmed_history)

        # 构造vLLM采样参数，映射配置
        temperature_val = float(temperature) if temperature > 0 else 1.0
        top_p_val = float(top_p) if temperature > 0 else 1.0

        sampling_params = SamplingParams(
            temperature=temperature_val,
            top_p=top_p_val,
            max_tokens=self.config.max_new_tokens,
            repetition_penalty=self.config.repetition_penalty if self.config.repetition_penalty > 1.0 else 1.0,
            # 其他参数可根据 vLLM 文档补充
        )
        # 线程锁确保安全
        with self._lock:
            try:
                # vLLM生成接口是同步的generate_or_iterate，获取完整生成文本
                response_generations = self.llm.generate(prompt, sampling_params)
                # 得到生成结果字符串
                response_text = ""
                for generation in response_generations:
                    response_text = generation.outputs[0].text
                    break  # 只取第一条结果
            except Exception as e:
                self.logger.error(f"vLLM推理失败: {e}", exc_info=True)
                response_text = "抱歉，生成失败，请稍后重试"

        trimmed_history.append({"role": "assistant", "content": response_text})
...
def run_ui_api(botagent: ChatBot, config: Config, logger: logging.Logger):
    ...
    async def root():
        return {
            ...
            "access": {
                "网页界面": f"http://{config.host}:{config.api_port}{config.ui_path}",
    ...
    app = gr.mount_gradio_app(app, gradio_demo, path=config.ui_path)
    # ========== 7. 启动 ==========
    logger.info("=" * 60)
    logger.info("  🚀 UI + API 组合服务启动")
    logger.info("=" * 60)
    logger.info(f"  🌐 网页界面:   http://{config.host}:{config.api_port}{config.ui_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ui-path", type=str, default=None, help="Gradio UI 挂载路径（仅 ui+api 模式）"
    )
    parser.add_argument(
...
def build_config_from_args(args: argparse.Namespace) -> Config:
    ...
    if args.ui_path is not None:
        config.ui_path = args.ui_path
