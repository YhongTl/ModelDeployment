"""
***大模型私有化部署服务---vLLM Docker API 模型uiapi完全版***
支持六种运行模式：cli / ui / api / ui+api /vllm docker api / vllm docker api + ui）
"""
...
import time  # 时间相关操作
import logging  # 日志记录
import httpx  # HTTP客户端库，用于调用 vLLM API
...
@dataclass
class Config:
    """全局配置类，集中管理所有可配置项，全局变量"""
    # ---------- 模型相关 ----------
    model_path = Path("vLLM Docker API 模型")  # 模型路径（vLLM Docker API 版本不直接使用本地模型路径，但保留此配置项以兼容接口，无需更改）
    max_context_length: int = 4096  # 模型最大上下文长度（包含输入和输出）
    max_new_tokens: int = 2048
    torch_dtype: str = "float16"  # float16 / bfloat16 / float32
    # ---------- 生成参数 ----------
    temperature: float = 0.6  # 设为 0 触发 do_sample=False，关闭采样，使用贪心解码，速度更快（但多样性下降）
    top_p: float = 0.9  # nucleus 采样的累积概率阈值，配合 temperature > 0 使用
    top_k: int = 50  # top-k 采样的 k 值，配合 temperature > 0 使用
    repetition_penalty: float = 1.1  # 重复惩罚，默认 1.0 表示不使用，>1.0 会惩罚重复生成的 token
    # ---------- 服务相关 ----------
    host: str = "0.0.0.0"  # 通配监听地址：监听所有网络接口，使得服务在局域网内可访问
    vllm_host: str = "127.0.0.1" # vLLM Docker API 监听地址（通常是本机）
    api_port: int = 8000  # FastAPI 服务端口
    ui_port: int = 7860  # Gradio UI 服务端口
    vllm_port: int = 8001  # vLLM Docker API 端口（ui-fastapi认证-才能调用vllm docker api推理）
...
# ================================================================
#              ChatBot 核心类（调用 vLLM Docker API 版本）
# ================================================================
class ChatBot:
    """基于 vLLM Docker API 的多轮对话机器人（无本地模型载入）"""
    def __init__(self, config: Config, logger: logging.Logger):
        self.config = config
        self.logger = logger
        self.model_path = str(config.model_path)  # 添加此行，提高健壮性
        self.api_url = f"http://{config.vllm_host}:{config.vllm_port}/v1/chat/completions"# 访问 vLLM Docker API 的基础地址
        self._lock = threading.Lock()  # 保证多线程调用线程安全
    ...
    def chat(
        ...
        """通过 HTTP POST 请求调用 vLLM API，发送消息并获取回复"""
        if history is None:
            history = []
        ...
        payload = {
            "model": "/models/DeepSeek-R1",  # vLLM Docker API 中预设的模型标识，需与 vLLM 配置一致curl http://127.0.0.1:8001/v1/models查看id
            "messages": history,
        ...
        with self._lock:
            try:
                ...
                return resp_text, history
            
            except Exception as e:
                self.logger.error(f"调用 vLLM API 失败: {e}", exc_info=True)
                try:
                    self.logger.error(f"响应体: {response.text}")
                except:
                    pass
                return "[调用 vLLM API 失败]", history
...
def build_gradio_app(
    ...
    def save_api_key(api_key):
        ...
        if not api_key:
            raise gr.Error("API Key 不能为空")
        return gr.update(visible=False), api_key

    def gradio_chat(
        user_message: str,
        chat_history: list,
        raw_history: list,        # 纯文本原始历史，用于传给模型
        api_key_input: str,
        api_key_saved: str,
    ):
        ...
        # 用 raw_history（纯文本）传给模型，而非 chat_history
        normalized = normalize_history(raw_history)
        ...
        # 更新原始历史（纯文本，供下一轮传给模型）
        new_raw_history = updated_history

        # 渲染显示历史（HTML，只给 chatbot 组件展示）
        display_history = []
        ...
        return "", display_history, new_raw_history

    def clear_chat():
        logger.info("对话历史已清空")
        return "", [], []  # 同时清空 raw_history
    ...
    with gr.Blocks(title="DeepSeek ChatBot") as demo:
        ...
        raw_history_state = gr.State(value=[])  # 原始历史状态
...
"""
        ...
        # ★ inputs/outputs 加入 raw_history_state
        send_btn.click(
            fn=gradio_chat,
            inputs=[user_input, chatbot, raw_history_state, api_key_input, api_key_state],
            outputs=[user_input, chatbot, raw_history_state],
        )
        user_input.submit(
            fn=gradio_chat,
            inputs=[user_input, chatbot, raw_history_state, api_key_input, api_key_state],
...
def run_api(botagent: ChatBot, config: Config, logger: logging.Logger):
    ...
    async def health_check():
        # 如果botagent实例中无model_path，改用配置中路径
        model_path_str = getattr(botagent, "model_path", None)
